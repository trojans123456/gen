...
class ResString
{
    public:
    //Methods
    explicit ResString( const QString &vl = "" );
    ~ResString( );

    ResString &operator=( const QString &val );
    operator QString( )		{ return getVal(); }

    size_t size( );
    bool   empty( );

    void setVal( const QString &vl );
    QString getVal( );
    const QString &getValRef( )	{ return str; }
